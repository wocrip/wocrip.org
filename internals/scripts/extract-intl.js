...
const i18n = require('../../app/services/intl/i18n')
const { DEFAULT_LOCALE } = require('../../app/services/intl/LanguageProvider/constants')
  ...
  try {
    ...
    for (const message of messages) {
      for (const locale of appLocales) {
        ...
        // Get local translation path
        const name = filename.split('/').pop()
        const filePath = filename.replace(name, '')
        const translationPath = `${filePath}translations/${locale}.json`
        // const translationPath2 = `${filePath}translations/${locale}.js`

        // Get local translation file and content
        let hasLocalTranslation = false
        let translation

        if (fs.existsSync(translationPath)) {
          const translationText = await readFile(translationPath)
          translation = JSON.parse(translationText)
          hasLocalTranslation = true
        }

        // Merge old translations into the babel extracted instances where react-intl is used
        const localeTranslation = hasLocalTranslation ? translation[message.id] : ''
        const newMsg = locale === DEFAULT_LOCALE
          ? message.defaultMessage
          : localeTranslation

        localeMappings[locale][message.id] =
          (oldLocaleMapping && oldLocaleMapping === newMsg)
            ? oldLocaleMapping
            : newMsg
      }
    }
